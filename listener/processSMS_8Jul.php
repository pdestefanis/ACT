	...
	class ProcessSMS  {
		...
		public function init($msg, $caller){
			...
			$validCodes = array(
								...
								1 => array("A", "ACCEPT", "ACEPTAR", "RECIBIR"), //UPPERCASE ONLY
								2 => array("D", "DELIVER"),
								3 => array("E", "EXPIRE"),
								4 => array("R", "RETURN"),
			...
			$phoneId = $this->getPhoneId($phoneNum);
				...
				if ($phoneId == -1) { //TODO Do we have this concept of inactive here?
					...
					$this->setRawreport(-1, $phoneNum, implode(" ", $message), $currDate, "phone number not found in database");
					error_log("[ ".date("Y-m-d H:i:s")." ]:  processSMS: PID not found: ".$action."\n", 3, "../parser.log");
