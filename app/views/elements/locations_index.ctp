<div class="search">
	...
	<tr<?php echo $class;?>>
		...
		<td><?php 
			if (isset($lev[$levels[$location['Location']['id']]]) && !empty ($lev[$levels[$location['Location']['id']]]))
				echo $lev[$levels[$location['Location']['id']]];
			else
				echo $levels[$location['Location']['id']]; 
			?>&nbsp;</td>
		...
		<td><?php  $access->checkHtml('Stats/view', 'text', $stats[$location['Location']['id']]['created'], '/stats/view/' . $stats[$location['Location']['id']]['sid'] ); ; ?>&nbsp;</td>
		<td class="actions">
		</td>
	</tr>
<?php endforeach; ?>
