...
class LocationsController extends AppController {
	...
	function index() {
		...
		$this->set('locations', $this->paginate(array( 'Location.deleted = 0')));
		...
		$stats= array();
		foreach (array_keys($this->Location->find('list')) as $p) {
			$st = $this->Location->Stat->find('list', array ('fields' => array('Stat.id', 'Stat.created'), 'conditions' => array('Stat.location_id = ' . $p)));
